...
const enDir = path.join(__dirname, 'lang')
...
gulp.task('sortEnglishKeys', function (done) {
  ...
  files.forEach(() => {
    const enFilePath = path.join(enDir, `en.json`)
...
gulp.task('less', function () {
  return gulp
    .src('./styling/less/*.less')
    .pipe(
      less({
        paths: [path.join(__dirname, 'less', 'includes')]
      })
    )
    .pipe(concat('ctd5e-styling.css')) // Concatenate all CSS files into a single file
...
gulp.task('watch-styling', function () {
  ...
  gulp.watch('./styling/less/*.less', gulp.series('less'))
})
